...
theme.paper <-  theme_minimal() +
                theme(strip.text.x = element_text(vjust=1,size = 18),
                      ...
                      plot.margin = unit(c(0,0,1.5,1.5), "cm"), # less margin on down side
...
het.all.plot <- ggplot(het.mums.df, aes(x=het,y=NumComps)) +
        ...
        #xlab("") +
        #ylab("Compound richness") 
ggsave(file="hetall.pdf", width=142, height=89.9, units=c("mm"),useDingbats=FALSE)
...
het.pc2.plot <- ggplot(het.mums.df,aes(x=het,y=PC2)) +
        ...
        #xlab("sMLH") +
        #ylab("PC2 score") 
ggsave(file="hetpc.pdf", width=142, height=89.9, units=c("mm"),useDingbats=FALSE)
...
pc.dens.plot.1 <- ggplot(pca.scores,aes(x=PC1)) +
        geom_density(alpha=0.8, size=0.5, aes(fill = beach)) +
        ...
        xlab("PC1 score") +
        ylab("Density")
 ggsave(file="PC1 scores.pdf", width=142, height=89.9, units=c("mm"),useDingbats=FALSE)
...
pc.dens.plot.2 <- ggplot(pca.scores,aes(x=PC2)) +
 ...
 ggsave(file="PC2 scores.pdf", width=142, height=89.9, units=c("mm"),useDingbats=FALSE)
