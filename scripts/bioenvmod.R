bioenvmod <- function (comm, env, method = "spearman", index = "bray", upto = ncol(env), 
       ...
       
{
        ...
        for (i in 1:upto) {
                ...
                for (j in 1:nrow(sets)) est[j] <- corfun(comdis, vegdist(x[, sets[j, ]],
                                                                         method = "bray"), partial, method = method)
