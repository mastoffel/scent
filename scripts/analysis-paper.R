...
relatedness <- as.matrix(read.csv(paste("C:\\Users\\Martin\\Studium\\",
                              "projects\\sealscent\\data_files\\",
                              "Rdata\\csv_files\\",
...
heterozygosity <- as.matrix(read.csv(paste("C:\\Users\\Martin\\Studium\\",
                                 "projects\\sealscent\\data_files\\",
                                 "Rdata\\csv_files\\",
...
# glms for association between heterozygosity and factors
library(minmodelr)

# backwards deletion testing 

# bind heterozygosity and the factor scores in one data.frame and subset mothers
hetdf <- cbind(heterozygosity, fa_scores)[factors$age == 1, ]
het_model <- glm(SH ~., data=hetdf)
summary(het_model)

#### here should be the model reduction via deletion testing ####

het_reduced <- glm(SH ~ F1 + F2, data=hetdf)
summary(het_reduced)

# sum of factors as variable
hetdf$F1F2 <- hetdf$F1 + hetdf$F2
summary(lm(SH ~ F1F2, data = hetdf))
