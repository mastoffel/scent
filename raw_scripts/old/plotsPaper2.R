...
theme.paper <-  theme_bw() +
                theme(strip.text.x = element_text(vjust=1,size = 18),
                      ...
                      plot.margin = unit(c(0,0,0.8,0.8), "cm"), # less margin on down side
...
theme.paper.2 <-  theme_bw() +
        theme(strip.text.x = element_text(vjust=1,size = 18),
              ...
              axis.text.x = element_text(size = 14),
              axis.text.y = element_text(size = 14),
              plot.margin = unit(c(0,0,0,0), "cm"), # less margin on down side
              # axis.line = element_line(size=1),
              # panel.border = element_blank() ,
              panel.grid.major = element_blank(),
              # axis.line = element_line(color = 'black'),
...
ggsave(file="hetall.svg", width=150, height=150, units=c("mm"))
...
het.pc2.plot <- ggplot(het.mums.df,aes(x=het,y=PC2)) +
        ...
        theme.paper.2 +    
        scale_x_continuous(breaks=c(seq(from = 0.8, to = 1.20, by = 0.1)))
ggsave(file="hetpc.svg", width=150, height=150, units=c("mm"))
