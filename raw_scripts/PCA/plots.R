...
het.all.plot <- ggplot(het.mums.df, aes(x=het,y=NumComps)) +
        ...
        geom_smooth(method="lm",size = 0.7 ,alpha=0.3, colour="blue") +
        ...
        theme(strip.text.x = element_text(vjust=1,size = 18),
              ...
              axis.text.y = element_text(size = 14),
              plot.margin = unit(c(1.5,1.5,0.1,1.5), "cm"), # less margin on down side
              # axis.line = element_line(size=1),
              # panel.border = element_blank() ,
              panel.grid.major = element_blank(),
              # axis.line = element_line(color = 'black'),
              panel.grid.minor = element_blank(),
              panel.border = element_rect(colour="black",fill=NA))+     
        scale_x_continuous(breaks=c(seq(from = 0.8, to = 1.15, by = 0.05)))+
...
het.pc2.plot <- ggplot(het.mums.df,aes(x=het,y=PC2)) +
        geom_point(colour = "black", size = 2) +
        geom_smooth(method="lm" ,size = 0.7 ,alpha=0.3,colour="blue") + #fill = "lightblue"
        #geom_point(size = 3,colour="grey", alpha=0.2, show_guide = F) +          
        theme_bw(base_size = 20) +
        theme(strip.text.x = element_text(vjust=1,size = 18),
              strip.background = element_rect(fill="white",linetype="blank"),
              axis.title.x = element_text(vjust=0.1,size = 16),
              axis.title.y = element_text(vjust=0.2,size = 16),
              axis.text.x = element_text(size = 14),
              axis.text.y = element_text(size = 14),
              plot.margin = unit(c(0.1,1.5,1.5,1.5), "cm"),
              # axis.line = element_line(size=1),
              # panel.border = element_blank() ,
              panel.grid.major = element_blank(),
              #axis.line = element_line(color = 'black'),
              ...
              panel.border = element_rect(colour="black",fill=NA))+     
        scale_x_continuous(breaks=c(seq(from = 0.8, to = 1.15, by = 0.05)))+
        #geom_text(aes(1,3, label="(b) r = 0.38, p = 0.016"),size=4) +
        xlab("heterozygosity") +
        ylab("PC2 score") 

ggsave(file="hetpc.svg", width=6, height=4)
...
pc.dens.plot <- ggplot(pca.scores,aes(x=PC2)) +
        geom_density(alpha=.5, size=1.2, aes(fill = beach)) +
...
ggplot(pc.plot.df.rel,aes(x=PC,y="mums")) +
        ...
        theme_bw(base_size=20) +
        theme(strip.text.x = element_text(vjust=1,size = 14),
              plot.title = element_text(vjust=1.1,face="bold",size=18),
